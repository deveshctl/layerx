...
Three correctness fixes in waste overlay, file viewer, and efficiency calculation. Plus two reliability fixes in the pull-progress and CI paths. Two extractor and analysis hardening fixes. One Windows-specific cache eviction fix. Ten low-priority correctness, robustness, and test-coverage fixes. Five further bug-scan fixes covering tar-path normalization, CI-rule disable semantics, pull-error propagation, file-viewer chroma cost, and extractor goroutine cancellation. Plus a tree-wide gopls cleanup: deprecated `tar.TypeRegA` removed, `client.NewClientWithOpts` migrated to `client.New`, and Go 1.22+ modernizations applied. A round-2 deep scan added 11 more fixes spanning hardlink metadata propagation, directory-to-file replacement diff emission, JSON-export tmp-file collisions, deterministic waste ordering, gzip reader leaks, file-viewer trailing-newline rendering, and version-string formatting. A round-3 review added six more, covering tar-path normalization in the extractor, silent JSON-export failures under `CI=true`, a duplicate Enter hint, the filetree below-indicator overwriting the filter-bar border, version-string built-suffix on missing dates, and the CI cross-compile matrix missing `arm64`. A round-4 deep scan adds 13 more, covering memory-bounded resolver parsing, end-to-end TUI quit cancellation, and a sweep of CRLF / title-truncation / CI-JSON-error-handling correctness fixes. A round-5 deep scan adds nine more, covering whiteout-then-readd correctness, save-to-disk safety (size cap, non-regular-file detection, async write, auto-rename), filter-input bounds, JSON-export tmp cleanup on rename failure, and config range validation. A round-6 deep scan adds 17 more, sweeping in-flight-request cancellation, status-bar message identity, finite-only config thresholds, strict YAML decoding, cache envelope hygiene, save-target uniqueness, CJK-aware truncation, viewer width clamping, and same-layer whiteout/readd resolution. A round-6 follow-up sweeps 11 more low-priority items: CLI threshold-flag validation, version-field sanitization, viewer-search length cap, two more `len([]rune)` → `lipgloss.Width` migrations, defensive nil-handling in `ci.Evaluate`, Windows-built-image backslash normalization in tar paths, non-regular-file rejection in two tar readers, and minor cosmetic / error-string cleanups.

### bug-batch — 2026-05-25 (round 6 follow-up)

Eleven low-priority items from the round-6 plan that didn't make the headline cut. Each verified against the current source.

- fix(cmd): CLI threshold flags (`--lowest-efficiency`, `--highest-wasted-bytes`, `--highest-user-wasted-percent`) are now validated before running. Out-of-range or non-finite values (`-0.5`, `1.5`, `NaN`, `Inf`) error out instead of silently disabling the rule via the existing `> 0` filter in `buildRules`. Validation runs for both `layerx ci ...` and `CI=true layerx ...` paths. Mirrors the config-file path that 08616e1 hardened.
- fix(cmd): `SetVersionInfo` now trims surrounding whitespace, strips embedded CR/LF/tab, and bounds each field to 64 chars before splicing into the version string. ldflags-injected values like ` unknown ` (trailing whitespace) previously defeated the sentinel guard; multi-line build metadata could splatter the `--version` output across multiple lines and confuse downstream parsers.
- fix(ci): `Evaluate` now treats a nil `*image.EfficiencyResult` as a zero-value result instead of panicking. The function is part of the public `ci` package API; defensive handling is cheap and prevents a NPE for callers that haven't run `image.Efficiency` yet.
- fix(tui): the file-viewer search input is now capped at the same `maxFilterLen=256` runes as the filter input, with the same partial-paste-fits semantics. A user holding down a key (or pasting a multi-megabyte string) into the viewer search previously grew `viewSearchQuery` unboundedly and re-walked every viewer line on each keystroke against an ever-larger needle.
- fix(tui): `renderTreeHeader` now measures with `lipgloss.Width` and slices with `ansi.Truncate` instead of `len(string)` and byte slicing. The header is currently ASCII so today no overflow can corrupt the cell, but the inconsistency with the b31296f migration would tripwire any future i18n of the column labels.
- fix(tui): `padHelpKey` now measures with `lipgloss.Width` instead of `len([]rune)`. Same i18n-tripwire shape as the tree header — the help table is currently ASCII so no visible bug today, but a future translated key label with CJK characters would have under-padded by 1–4 cells.
- fix(tui): `formatWasteRow` no longer Sprintf's `"L0"` for a negative `IntroLayer` before overriding to `"L?"`. Cosmetic — both branches produced the same final text — but the dead allocation was confusing on read.
- fix(image): `cleanTarPath` now normalizes backslashes to forward slashes. Windows-native build tooling has historically emitted tar entries like `etc\\foo`; downstream tree insertion (which splits on `/`) previously created a single child named `etc\foo` instead of nesting `foo` under `etc`. Rare today, but the cost is one `strings.ReplaceAll` and the failure mode is silent.
- fix(image): the `errPathNotRegular` surface error in `ExtractFromLayer` / `ExtractRawFromLayer` no longer redundantly wraps the sentinel into the formatted message (`"... (path exists but is not a regular file: typeflag N)"`). The user-facing string read awkwardly with two parenthetical clauses; the typeflag detail was never actionable for an end user. Internal `errors.Is(err, errPathNotRegular)` checks at the call sites are unaffected (the wrap was at the *outer* error, not the chain).
- fix(image): `readFirstFileFromTar` and `readFullFileFromTar` now skip every non-regular-file tar entry instead of just `tar.TypeDir`. Symlink / hardlink / device / fifo entries previously flowed into the body-read path; for `CopyFromContainer` (single-entry tar) this manifested as zero-length reads of symlink targets surfacing as "the file" — confusing for the file viewer, wrong for save-to-disk. Aligns the contract with the docstring: "first regular file in the stream".
- docs(cmd): the `rootCmd` Long help (added in round 6) now lives next to the new validateCLIThresholdFlags helper, so the equivalence between `CI=true layerx ...` and `layerx ci ...` is enforced both in documentation and in error-handling.

### bug-batch — 2026-05-25 (round 6)

Round 6 deep scan: three parallel agents reviewed every Go file and recent diffs; 19 verified findings produced 17 actionable fixes (R6-2 took the documentation path; R6-17 deferred as cosmetic).

- fix(tui): per-request file-viewer and save contexts now derive from `m.fetchCtx`, and a shared `cancelInflight()` helper is invoked at every quit site (Esc/q/Ctrl-C in the main view and the waste overlay). Quitting mid-`Enter` (extract) or mid-`x` (save) previously left the Docker `CopyFromContainer` HTTP request running for many seconds because `viewerCancel` and `saveCancel` were rooted at `context.Background()` and never propagated from the fetch-cancel. The "process hangs after Ctrl+C" symptom is gone.
- fix(tui): `saveFileCmd` now takes a `context.Context`, the probe loop in `uniquePath` checks `ctx.Err()` between candidates, and the save phase re-arms `m.saveCancel` against a fresh child of `m.fetchCtx`. A quit during the stat/write phase on a slow filesystem (network mount, encrypted volume) now aborts within one probe instead of blocking the goroutine.
- fix(tui): `uniquePath` now returns an error when all 1000 `.N` candidates exist, instead of silently returning `<base>.999<ext>` and clobbering whatever already lives there. Save-target generation can no longer overwrite a pre-existing file by exhausting the suffix space.
- fix(tui): status-bar messages now carry an identity. Each set bumps `m.statusGen` via a new `setStatus(...)` helper; each scheduled `clearStatusMsg` captures the gen and is ignored on dispatch if a newer message has overwritten the original. Older 2-second clear ticks no longer erase a freshly-set message inside the window (e.g. a "Collapse unavailable" message followed within 2 seconds by "Extracting…" no longer flashes blank mid-extract).
- fix(tui): layer-command and filename truncation now measure with `lipgloss.Width` and slice with `ansi.Truncate` instead of `len([]rune)` and rune slicing. CJK and other wide-cell content previously passed the fits-check on rune count but rendered overflowing the cell budget, forcing the renderPanel safety net to lop the trailing diff colour or origin suffix.
- fix(tui): `renderPanel` now clamps a negative `contentWidth` or `height` to 0 and `viewLoading` short-circuits with a plain "loading…" message at widths under 10. Either path previously crashed via `strings.Repeat("─", -1)` on a 1-cell terminal.
- fix(tui): the spinner `tea.Cmd` is no longer dispatched twice on `tryOpenSelectedFile`. `Init` already started the tick chain; the duplicate from the file-open `tea.Batch` doubled the animation rate and grew the cmd queue per file-open cycle.
- fix(tui): the file-tree filter input now accepts the prefix that fits when a paste exceeds `maxFilterLen=256` instead of dropping the whole paste. Pasting 1000 chars into an empty filter now leaves 256 chars (the cap), not 0.
- fix(tui): the file viewer and chroma highlighter now strip bare `\r` after the CRLF→LF normalization pass. Mac classic line endings or stray CRs in mixed-source files previously reset the terminal cursor mid-row, smashing the panel border for the rest of the frame.
- fix(image): `findFileInLayer` now walks the full layer tar and prefers a regular-file entry over an earlier `.wh.<seg>` for the same path within a single layer. Overlayfs upper-layer semantics permit `.wh.X` followed by `X` in one layer (the readd shadows the deletion) — the previous early-return on first-hit incorrectly surfaced "removed" for the readded file. Mirrors the opaque-whiteout dedup already in `Stack`.
- fix(image): `Stack.extractWhiteouts` now skips reserved control entries with the `.wh..wh.` prefix (other than `.wh..wh..opq`) instead of treating them as regular whiteouts of `.wh.<x>`. The previous code emitted phantom Removed entries for non-existent paths.
- fix(image): `tree_parser.insertNode` now promotes a regular-file node to a directory when a later tar entry adds a child path beneath it (`/etc/foo` then `/etc/foo/bar`). Symmetric to the dir→file promotion added in b779099. Tree state can no longer end up with `IsDir=false` AND `len(Children)>0`.
- fix(image): `loadCache` now rejects an envelope whose `Layers` list is empty and removes the file. A cache that decodes successfully to zero layers is impossible for a real manifest; previously the TUI presented an empty image with no signal that the cache was the cause.
- fix(image): `isTransientIOError` no longer blanket-classifies every `*os.PathError` as transient. Genuinely transient `syscall.Errno` values (EAGAIN, EBUSY, EINTR, EMFILE, ENFILE, ENOSPC) plus `os.ErrDeadlineExceeded` are the only paths that warrant retry; `EACCES` / `EPERM` / `ENOENT` no longer trigger silent retries that mask configuration errors.
- fix(ci): `LowestEfficiency.Evaluate` now treats `Threshold <= 0` and `NaN` scores as a pass, mirroring the disable semantics already used by the other two rules. The asymmetry could surface if a third-party caller used the rule directly (`buildRules` filtered threshold-zero before reaching `Evaluate`, masking it for the in-tree call site).
- fix(config): `validate()` now rejects `NaN` and `±Inf` for `lowest-efficiency` and `highest-user-wasted-percent`. A YAML body with `.nan` or `.inf` previously parsed cleanly through validation and produced a confusing `"Actual: NaN"` rule failure on every CI run.
- fix(config): YAML decoding is now strict (`yaml.NewDecoder(..., yaml.Strict())`). A typo like `lowst-efficiency: 0.95` (missing `e`) or `highest_wasted_bytes` (snake-case) previously parsed silently and the user wondered why the override never took effect; the loader now errors with the unknown field name.
- docs(cmd): `rootCmd` Long now states explicitly that `CI=true` runs ci with default (or config-file) thresholds and that command-line threshold overrides require `layerx ci ...` directly. Resolves the misleading suggestion that `CI=true layerx --lowest-efficiency 0.95 IMG` would work (the threshold flags are registered on `ciCmd`, not `rootCmd`).

### bug-batch — 2026-05-25 (round 5)

Round 5 deep scan: 21 candidate findings triaged; 9 actionable fixes verified by direct source inspection. Skipped items were either by-design or duplicates of earlier rounds.

- fix(image): `Stack` now de-dupes when an opaque whiteout is paired with a re-introduction of the same name in the same layer (`/foo` removed via `.wh..wh..opq` and a fresh `/foo` added). The previous code emitted both a `Removed` clone and an `Added` clone for the re-introduced path, so the stacked tree carried two children with the same name and the file viewer / waste overlay double-counted them.
- fix(image): `readFullFileFromTar` now caps both header-advertised size and actual bytes read at `MaxSaveSize` (2 GiB). Save-to-disk previously called this with no size limit, so a 100 GB tar entry — legal in the format and observed in some BuildKit cache layers — would allocate 100 GB or OOM the process before any error surfaced. Header size is checked before allocation; body read goes through `io.LimitReader`.
- fix(image): `findFileInLayer` now returns a typed `errPathNotRegular` error when a tar entry at the requested path is not a regular file (symlink, directory, char/block device). Previously the `Typeflag != tar.TypeReg` check returned `(nil, false, nil)` — indistinguishable from "not in this layer" — so `ExtractFromLayer` walked back and silently returned the older regular-file bytes from an earlier layer. The walk-back now treats `errPathNotRegular` as terminal and surfaces a "not a regular file" error to the caller.
- fix(tui): `padRight` / `padLeft` now measure with `lipgloss.Width` and truncate with `ansi.Truncate` instead of byte length and byte slicing. Filenames with CJK characters, accented Latin, or emoji previously over- or under-padded by 1–4 cells per row, mis-aligning the size and uid/gid columns; truncation could also slice mid-rune and corrupt subsequent cells.
- fix(tui): file save (`x` key) now (a) writes off the UI thread via a `tea.Cmd` so a slow disk no longer freezes the TUI for the duration of the write, and (b) auto-renames to `name.1`, `name.2`, … when the target already exists, instead of silently overwriting an existing file in the working directory. The status line distinguishes the two paths: `Saved: <name>` vs `Saved: <name> (existed → wrote <new-name>)`. Stale dispatches (from a second `x` press during the first write) are dropped via a `requestID` guard.
- fix(tui): `renderWasteOverlay` now calls `wasteVisibleHeight()` instead of repeating the `max(m.height-10, 1)` arithmetic inline. The two computations had silently drifted in earlier rounds (key-handler used the helper, renderer did not) — any future tweak to overlay chrome would have skewed scroll math against rendered rows. Receiver normalized to value (`m model`) for consistency with the renderer.
- fix(tui): the file-tree filter input is now capped at 256 runes. A user holding down a key (or pasting a multi-megabyte string) could grow `filterQuery` unboundedly; every keystroke then re-walked the tree and ran `strings.Contains` against an ever-larger needle, freezing the TUI.
- fix(cmd): `writeJSONAtomic` now removes its temp file when `os.Rename` fails (cross-device rename, target is a directory, target's parent is read-only). The previous code returned the rename error and left the `.layerx-json-*.tmp` file behind; repeated runs accumulated litter in the export directory.
- fix(config): `LoadFrom` now validates threshold ranges. `lowest-efficiency` and `highest-user-wasted-percent` must be in `[0, 1]`; `highest-wasted-bytes` must be `>= 0`. A typo like `lowest-efficiency: 95` (meant to be `0.95`) previously parsed cleanly and caused every CI run to fail with a confusing "below threshold 95.000" message.
...
- fix(tui): the pull-progress bytes-total ("361.0 MB", "4.7 GB") could still be clipped at the right border on certain widths because the bar-budget calculation reserved 0 inner-padding cells, leaving the line equal to `boxWidth` (which `renderPanel` truncates). Budget now reserves 2 cells (`m.width - 6`) so the bytes total stays inside the box across all widths from 40 cols up. Also clamps `pct` to 100 to avoid a negative-repeat when Docker briefly reports `BytesCurr > BytesTotal`.

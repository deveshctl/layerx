...
func TestErrPlatformNotInImage_Format(t *testing.T) {
	...
	// Without an available list (legacy image store, older daemon, or the
	// manifest-list inspect failed), the error stays terse — we don't guess
	// at what the image carries when the daemon does not surface it.

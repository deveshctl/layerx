...
type Config struct {
	...
	// not this field.
	Platform string
	// Theme is the resolved theme name; "" means the default (mocha). The
	// name is validated in cmd/ before reaching here; NewModel falls back to
	// the default rather than crash if an unknown name slips through.
	Theme string
}
...
func NewModel(cfg Config) model {
	...
	// cfg.Theme is validated in cmd/ before reaching here; treat a late
	// error defensively by falling back to the default rather than crashing
...
func (m model) renderRightPanel(width, height int) string {
	...
	return renderSplitFileTree(m.theme, splitTreeInput{
		width:        width,
		height:       height,
...
func (m model) renderStatusBar(treeFiles []*image.FileNode) string {
	...
	var right string
	if m.statusMsg != "" {
		msgStyle := lipgloss.NewStyle().Foreground(m.theme.DiffModify).Background(m.theme.StatusBg).Bold(true)
		right = msgStyle.Render(m.statusMsg) + " "
	} else if m.copyConfirm {
		copiedStyle := lipgloss.NewStyle().Foreground(m.theme.DiffAdd).Background(m.theme.StatusBg).Bold(true)
		...
		if m.diffOnly {
			badges += lipgloss.NewStyle().Foreground(m.theme.DiffModify).Background(m.theme.StatusBg).Render("[diff]") + " "
		}
		if m.aggregated {
...
func (m model) renderViewerStatusBar() string {
	...
	if m.copyConfirm {
		...
		matchStyle := lipgloss.NewStyle().Foreground(m.theme.Accent).Background(m.theme.StatusBg).Bold(true)
		...
		rightDim := lipgloss.NewStyle().Foreground(m.theme.TextDim2).Background(m.theme.StatusBg)

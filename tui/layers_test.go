...
func testStyles() Styles {
	return BuildStyles(theme.Default().Palette)
}
...
func TestFormatLayerLine_DeltaMode_HasSignedColumn(t *testing.T) {
	...
	out := formatLayerLine(testStyles(), l, false, 60, sizeColDelta, 30*mb)
...
func TestFormatLayerLine_BlobMode_RegressionGuard(t *testing.T) {
	const mb = 1024 * 1024
	l := makeLayer(1, 5*mb, 3*mb, "RUN apt-get install foo")
	out := formatLayerLine(testStyles(), l, false, 60, sizeColBlob, 30*mb)
...
func TestFormatLayerLine_BothMode_HasBothValues(t *testing.T) {
	const mb = 1024 * 1024
	l := makeLayer(1, 5*mb, 3*mb, "RUN apt-get install foo")
	out := formatLayerLine(testStyles(), l, false, 80, sizeColBoth, 30*mb)
	assert.Contains(t, out, "5.0 MB")
	assert.Contains(t, out, "+3.0 MB")
}
...
func TestRenderLayers_BothMode_FallsBackToDelta_OnNarrowPanel(t *testing.T) {
	...
	out := renderLayers(testStyles(), layers, 0, 0, 36, 10, true, sizeColBoth, 8*mb)
...
func TestRenderLayers_FocusedTitle_ReflectsSizeMode(t *testing.T) {
	...
	out := renderLayers(testStyles(), layers, 0, 0, 60, 5, true, sizeColDelta, 1000)
	assert.Contains(t, out, sizeModeLabelChange)

	out = renderLayers(testStyles(), layers, 0, 0, 60, 5, true, sizeColBlob, 1000)
	...
	out = renderLayers(testStyles(), layers, 0, 0, 60, 5, true, sizeColBoth, 1000)

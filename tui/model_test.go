...
func TestRenderLayersDoesNotPanic(t *testing.T) {
	...
	assert.NotPanics(t, func() {
		renderLayers(testStyles(), a.Layers, 0, 0, 40, 20, true, sizeColDelta, 0)
	})
}

func TestRenderLayersEmptyDoesNotPanic(t *testing.T) {
	assert.NotPanics(t, func() {
		renderLayers(testStyles(), nil, 0, 0, 40, 20, false, sizeColDelta, 0)
	})
}
...
func TestRenderFileTreeDoesNotPanic(t *testing.T) {
	...
	files := m.displayTree()
	assert.NotPanics(t, func() {
		renderFileTree(testStyles(), files, 0, 0, 60, 20, true, false, "", true, nil, 0)
	})
}

func TestRenderFileTreeEmptyShowsPlaceholder(t *testing.T) {
	output := renderFileTree(testStyles(), nil, 0, 0, 60, 20, false, false, "", true, nil, 0)
...
func TestRenderFileTreeCollapsedDirShowsGlyph(t *testing.T) {
	...
	um := updated.(model)
	line := renderFileTree(testStyles(), um.displayTree(), um.treeCursor, 0, 80, 20, true, false, "", true, um.treeCollapsed, 0)

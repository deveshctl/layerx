...
func TestRenderLayersDoesNotPanic(t *testing.T) {
	...
	assert.NotPanics(t, func() {
		renderLayers(defaultTheme(), a.Layers, 0, 0, 40, 20, true, sizeColDelta, 0)
	})
}

func TestRenderLayersEmptyDoesNotPanic(t *testing.T) {
	assert.NotPanics(t, func() {
		renderLayers(defaultTheme(), nil, 0, 0, 40, 20, false, sizeColDelta, 0)
	})
}
...
func TestRenderFileTreeDoesNotPanic(t *testing.T) {
	...
	files := m.displayTree()
	assert.NotPanics(t, func() {
		renderFileTree(defaultTheme(), files, 0, 0, 60, 20, true, false, "", true, false, nil, 0)
	})
}

func TestRenderFileTreeEmptyShowsPlaceholder(t *testing.T) {
	output := renderFileTree(defaultTheme(), nil, 0, 0, 60, 20, false, false, "", true, false, nil, 0)
...
func TestRenderFileTreeCollapsedDirShowsGlyph(t *testing.T) {
	...
	um := updated.(model)
	line := renderFileTree(defaultTheme(), um.displayTree(), um.treeCursor, 0, 80, 20, true, false, "", true, false, um.treeCollapsed, 0)
